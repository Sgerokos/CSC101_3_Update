...
def ordered_Quantity(item_name, entree_num):
    # Loop until valid input is received
    while True:
        try:
            # Ask the user for the quantity of the item they want to order
            amount = int(input(f"How many {item_name} would you like to order? "))
            if amount <= 0:
                # Handle invalid input (amount must be positive)
                print("Please enter a positive number.")
            else:
                # Add the quantity to the ordered quantity list and return
                main.ordered_Quantity.append(amount)
                print(f"{amount} {item_name}(s) added to your order.\n")
                return
        except ValueError:
            # Handle invalid input (input must be a valid integer)
            print("Please enter a valid integer.")

def confirmation(menu_num, menu_items, menu_prices, entree_num):
    ...
    if add_to_order == "y":
        ...
        index = menu_num.index(entree_num)
        ...
        ordered_Quantity(menu_items[index], entree_num)
        side()
    ...
    # Handle invalid user input
        ...
        return confirmation(menu_num, menu_items, menu_prices, entree_num)
...
def menu():
    ...
    # Call confirmation function with the selected entree number and corresponding menu items and prices
    confirmation(menu.ordered_Items, menu.menu_Items, menu.menu_Prices, menu.entrees)
